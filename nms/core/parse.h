...
template<class T>
__forceinline auto _parse_switch(StrView buf, StrView fmt, T& t, Version<3>) -> decltype(t.parse(buf, fmt), 0) {
    t = t.parse(buf, fmt);
    return 0;
}

template<class T>
__forceinline auto _parse_switch(StrView buf, StrView fmt, T& t, Version<2>) -> decltype(t.parse(buf), 0) {
    t = t.parse(buf);
    return 0;
}

template<class T>
__forceinline auto _parse_switch(StrView buf, StrView fmt, T& t, Version<0>) ->decltype(parseImpl(buf, fmt, t), 0) {
    parseImpl(buf, fmt, t);
    return 0;
}
...
__forceinline auto parse_switch(StrView buf, StrView fmt, T& t) {
    return _parse_switch(buf, fmt, t, Version<3>{});
}
