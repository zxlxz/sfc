...
class [[nodiscard]] List {
 ...
 public:
  ...
  auto operator[](usize idx) const noexcept -> const T& {
    sfc::assert_fmt(idx < _len, "List::[]: idx(={}) out of ids(={})", idx, _len);
    return _inn[idx];
  }
 ...
 public:
  ...
  auto swap_remove(usize idx) noexcept -> T {
    sfc::assert_fmt(idx < _len, "List::swap_remove: idx({}) out of ids([0,{}))", idx, _len);
  ...
  void insert(usize idx, T val) noexcept {
    sfc::assert_fmt(idx <= _len, "List::insert: idx(={}) out of ids([0,{}))", idx, _len);
  ...
  auto remove(usize idx) noexcept -> T {
    sfc::assert_fmt(idx < _len, "List::remove: idx(={}) out of ids([0,{}))", idx, _len);
