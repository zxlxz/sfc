...
class [[nodiscard]] Queue {
 ...
 public:
  auto operator[](usize idx) const -> const T& {
    ...
    panicking::assert(idx < len, "Queue::[]: idx(={}) out of range(={})", idx, len);
  ...
  auto first() const -> const T& {
    panicking::assert(!this->is_empty(), "Queue::first: queue is empty");
    return _vec[_pos];
  }

  auto first_mut() -> T& {
    panicking::assert(!this->is_empty(), "Queue::first_mut: queue is empty");
    return _vec[_pos];
  }

  auto last() const -> const T& {
    panicking::assert(!this->is_empty(), "Queue::last: queue is empty");
    return _vec[_vec.len() - 1];
  }

  auto last_mut() -> T& {
    panicking::assert(!this->is_empty(), "Queue::last_mut: queue is empty");
    return _vec[_vec.len() - 1];
  }
