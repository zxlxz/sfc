...
SFC_TEST(utf8_encode) {
  ...
  sfc::assert_eq(chr::utf8_encode(buf, 0x10000), 4U);
  sfc::assert_eq(buf[0], 0xF0);
  ...
  sfc::assert_eq(chr::utf8_encode(buf, 0x10FFFF), 4U);
  ...
  sfc::assert_eq(chr::utf8_encode(buf, 0x110000), 0U);
}
...
SFC_TEST(utf16_encode) {
  ...
  // BMP
  sfc::assert_eq(chr::utf16_encode(buf, U'A'), 1U);
  sfc::assert_eq(buf[0], u16{'A'});

  sfc::assert_eq(chr::utf16_encode(buf, U'中'), 1U);
  sfc::assert_eq(buf[0], u16{0x4E2D});

  // Supplementary: U+1F600 (😀)
  sfc::assert_eq(chr::utf16_encode(buf, 0x1F600), 2U);
  sfc::assert_eq(buf[0], u16{0xD83D});
  sfc::assert_eq(buf[1], u16{0xDE00});

  // Boundary: U+10000
  sfc::assert_eq(chr::utf16_encode(buf, 0x10000), 2U);
  sfc::assert_eq(buf[0], u16{0xD800});
  sfc::assert_eq(buf[1], u16{0xDC00});

  // Boundary: U+10FFFF
  sfc::assert_eq(chr::utf16_encode(buf, 0x10FFFF), 2U);
  sfc::assert_eq(buf[0], u16{0xDBFF});
  sfc::assert_eq(buf[1], u16{0xDFFF});

  // Invalid
  sfc::assert_eq(chr::utf16_encode(buf, 0x110000), 0U);
}
