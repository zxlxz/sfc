  ...
  void fmt(auto& f) const {
    if constexpr (requires { _val.fmt(f); }) {
      ...
      if constexpr (requires { to_str(_val); }) {
        ...
        f.pad_num(false, num::int2str(buf, static_cast<U>(_val), f.style()._type));
      }
      ...
      char buf[8 * sizeof(_val)];
      f.pad_num(_val < 0, num::int2str(buf, _val < 0 ? -_val : _val, f.style()._type));
    } else if constexpr (__is_same(decltype(_val + 0ULL), unsigned long long)) {
      char buf[8 * sizeof(_val)];
      f.pad_num(false, num::int2str(buf, _val + 0U, f.style()._type));
    }
  }
};
