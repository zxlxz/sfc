...
struct Slice<const T> {
 ...
 public:
  [[gnu::always_inline]] auto operator[](usize idx) const noexcept -> const T& {
    sfc::assert_fmt(idx < _len, "Slice::[]: idx(={}) out of range(={})", idx, _len);
    return _ptr[idx];
  }
...
struct Slice {
 ...
 public:
  void swap(usize i, usize j) noexcept {
    sfc::assert_fmt(i < _len, "Slice::[]: i(={}), out of range(={})", i, _len);
    sfc::assert_fmt(j < _len, "Slice::[]: j(={}), out of range(={})", j, _len);
