...
SFC_TEST(utf8_codelen) {
  // ASCII (0xxxxxxx)
  sfc::expect_eq(chr::utf8_codelen(0x00), 1U);
  sfc::expect_eq(chr::utf8_codelen(0x41), 1U);
  sfc::expect_eq(chr::utf8_codelen(0x7F), 1U);

  // 2-byte (110xxxxx)
  sfc::expect_eq(chr::utf8_codelen(0xC2), 2U);
  sfc::expect_eq(chr::utf8_codelen(0xDF), 2U);

  // 3-byte (1110xxxx)
  sfc::expect_eq(chr::utf8_codelen(0xE0), 3U);
  sfc::expect_eq(chr::utf8_codelen(0xEF), 3U);

  // 4-byte (11110xxx)
  sfc::expect_eq(chr::utf8_codelen(0xF0), 4U);
  sfc::expect_eq(chr::utf8_codelen(0xF4), 4U);

  // Invalid: continuation byte (10xxxxxx)
  sfc::expect_eq(chr::utf8_codelen(0x80), 0U);
  sfc::expect_eq(chr::utf8_codelen(0xBF), 0U);

  // Invalid: too high (11111xxx)
  sfc::expect_eq(chr::utf8_codelen(0xF8), 0U);
  sfc::expect_eq(chr::utf8_codelen(0xFF), 0U);
}
...
SFC_TEST(utf8_decode) {
  auto decode = [](Slice<const char8_t> s) {
    const auto p = reinterpret_cast<const u8*>(s._ptr);
    return chr::utf8_decode(p, s._len - 1);
  };

  // 1-byte
  sfc::expect_eq(decode(u8"A"), char32_t{'A'});

  // 2-byte: é (0xC3 0xA9)
  sfc::expect_eq(decode(u8"é"), char32_t{U'é'});

  // 3-byte: 中 (0xE4 0xB8 0xAD)
  sfc::expect_eq(decode(u8"中"), char32_t{U'中'});

  // 4-byte: 😀 (0xF0 0x9F 0x98 0x80)
  sfc::expect_eq(decode(u8"😀"), char32_t{0x1F600});

  // Invalid n=0
  sfc::expect_eq(decode(u8""), chr::INVALID);
}

SFC_TEST(utf16_codelen) {
  // BMP (< 0xD800)
  sfc::expect_eq(chr::utf16_codelen(0x0000), 1U);
  sfc::expect_eq(chr::utf16_codelen(0x0041), 1U);
  sfc::expect_eq(chr::utf16_codelen(0xD7FF), 1U);

  // High surrogate range → 2 (surrogate pair)
  sfc::expect_eq(chr::utf16_codelen(0xD800), 2U);
  sfc::expect_eq(chr::utf16_codelen(0xDBFE), 2U);

  // Low surrogate range → 0 (invalid start)
  sfc::expect_eq(chr::utf16_codelen(0xDC00), 0U);
  sfc::expect_eq(chr::utf16_codelen(0xDFFF), 0U);

  // After surrogates
  sfc::expect_eq(chr::utf16_codelen(0xE000), 1U);
  sfc::expect_eq(chr::utf16_codelen(0xFFFF), 1U);
}
...
SFC_TEST(utf16_decode) {
  auto decode = [](Slice<const char16_t> s) {
    const auto p = reinterpret_cast<const u16*>(s._ptr);
    return chr::utf16_decode(p, s._len - 1);
  };

  sfc::expect_eq(decode(u"A"), char32_t{U'A'});
  sfc::expect_eq(decode(u"😀"), char32_t{0x1F600});
  sfc::expect_eq(decode(u""), chr::INVALID);
}
