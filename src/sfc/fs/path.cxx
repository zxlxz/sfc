...
SFC_TEST(push) {
  ...
  p.push("foo.txt");
  panicking::expect_eq(p.as_path(), Path{"tmp/foo.txt"});
}
...
SFC_TEST(pop) {
  ...
  p.pop();
  panicking::expect_eq(p.as_path(), Path{"tmp"});
}

SFC_TEST(join) {
  const auto p = Path{"tmp"};
  panicking::expect_eq(p.join("foo.txt").as_path(), Path{"tmp/foo.txt"});
}

SFC_TEST(set_file_name) {
  auto p = PathBuf::from("tmp/foo.txt");
  p.set_file_name("bar.txt");
  panicking::expect_eq(p.as_path(), Path{"tmp/bar.txt"});
}

SFC_TEST(set_extension) {
  auto p = PathBuf::from("tmp/foo.txt");
  p.set_extension("md");
  panicking::expect_eq(p.as_path(), Path{"tmp/foo.md"});
}
