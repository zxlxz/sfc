...
class Mutex {
 ...
 public:
  ...
  void lock() {
    ...
    sfc::assert_fmt(err == 0, "sync::Mutex::lock: failed, err={}", err);
  }

  void unlock() {
    const auto err = ::pthread_mutex_unlock(_raw);
    sfc::assert_fmt(err == 0, "sync::Mutex::unlock: failed, err={}", err);
  }

  auto try_lock() -> bool {
    const auto err = ::pthread_mutex_trylock(_raw);
    sfc::assert_fmt(err == 0 || err == EBUSY, "sync::Mutex::try_lock: failed, err={}", err);
    return err == 0;
  }
};
...
class Condvar {
 ...
 public:
  ...
  void notify_one() {
    ...
    sfc::assert_fmt(err == 0, "sync::Condvar::notify_one: failed, err={}", err);
  }

  void notify_all() {
    const auto err = ::pthread_cond_broadcast(_raw);
    sfc::assert_fmt(err == 0, "sync::Condvar::notify_all: failed, err={}", err);
  }

  void wait(Mutex& mtx) {
    const auto err = ::pthread_cond_wait(_raw, mtx._raw);
    sfc::assert_fmt(err == 0, "sync::Condvar::wait: failed, err={}", err);
  }
  ...
  auto wait_timeout(Mutex& mtx, time::Duration dur) -> bool {
    ...
    if (::clock_gettime(CLOCK_ID, &ts) == -1) {
      ...
      sfc::assert_fmt(false, "sync::Condvar::wait_timeout: clock_gettime failed, err={}", err);
    }
    ...
    sfc::assert_fmt(err == 0 || err == ETIMEDOUT, "sync::Condvar::wait_timeout: failed, err={}", err);
    return err == 0;
  }
};
